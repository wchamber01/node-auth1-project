...
router.post("/login", (req, res) => {
  ...
  Users.findBy({ username })
    .first()
    .then(user => {
      console.log(req.session, "req.session line 39");
      if (!user) {
...
router.get("/logout", (req, res) => {
  console.log(req.session, "req.session line 56");
  if (req.session) {
    ...
    //   });
    // } else {
    //   console.log(req.session, "req.session line 65");
    res.status(200).json({
      message: "You don't have to go home but you can't stay here!"
    });
    //     }
    //   });
  } else {
